...
public:
  ...
  size_type max_size() { return  461168601842738790; }
  ...
  void set_copy(node_p other) {
    ...
    if (other->left_ != other->get_nill())
      this->set_copy(other->left_);
    if (other->right_ != other->get_nill())
      this->set_copy(other->right_);
  }
...
private:
  ...
  void rebuild_set(node_p base, node_p nill_) {
    ...
    } else if (base == base->parent_->right_) { // если удаляется правый потомок
      if (base->right_ != nill_) {
